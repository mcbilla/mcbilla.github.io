...
### 2）标题（title）
Hexo 默认以标题做为文件名称，可以设置配置文件的 `new_post_name` 参数来改变默认的文件名称，例如设为 `:year-:month-:day-:title.md` ，文件名会带上年月日，其中 :title 默认会转为小写，空格将会被替换为短杠
...
文章编辑可以用普通的markdown编辑器，例如本人习惯使用typora编辑器。

文章内容分为前页（Front-matter）和正文
