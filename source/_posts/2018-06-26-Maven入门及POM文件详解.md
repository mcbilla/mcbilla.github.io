...
### Maven 生命周期
参考 [Maven Lifecycle](http://maven.apache.org/guides/introduction/introduction-to-the-lifecycle.html)，Maven 项目的生命周期大概可分为以下阶段：

* validate：校验阶段，校验项目的正确性和必要配置信息是否可用。
* compile：编译阶段
* test：单元测试阶段，单元测试代码不会被 `package` 和 `deploy`
* package：打包阶段，打包成指定格式如 jar 包
* verify：集成测试阶段
* install：将 jar 包发布到本地仓库
* deploy：将 jar 包发布到远程仓库

### Maven 常用命令
参考 [Maven Plugins](http://maven.apache.org/plugins/index.html)，Maven 实际上是一个插件的集成框架，Maven 的所有任务都是通过插件来完成的。Maven 包含核心插件和其他插件，其中两种类型的插件可以在 `pom.xml` 文件配置相关信息：
